...
export default function ShopifyAuthPage() {
  ...
  useEffect(() => {
    ...
    const shop = params.get('shop') || 'tkww3m-6u.myshopify.com';
  ...
  const handleStartAuth = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const shop = fd.get('shop') as string || 'tkww3m-6u.myshopify.com';
    ...
    try {
      ...
      if (data.access_token) {
        ...
        setErrorMsg(data.error || 'Failed to get Shopify access token');
      }
    } catch (err) {
  ...
  return (
    <>
      <Header title="Shopify Token Assistant" subtitle="Công cụ kết nối Shopify bằng Client ID/Secret" />
      <PageContainer>
        <div className="card" style={{ maxWidth: 800, margin: '0 auto' }}>
          
          {accessToken ? (
            <div style={{ textAlign: 'center', padding: 'var(--space-xl)' }}>
              <h2 style={{ color: 'var(--color-winner)', marginBottom: 'var(--space-md)' }}>✅ Kết nối thành công!</h2>
              <p style={{ color: 'var(--text-muted)', marginBottom: 'var(--space-md)' }}>
                Token đã được lưu vào AdPilot. Shopify Dev Dashboard token có hạn 24h, hệ thống sẽ tự lấy lại bằng Client ID/Secret khi cần.
              </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
              ...
              <p style={{ color: 'var(--text-muted)' }}>
                Hãy điền Client ID và Client Secret từ Shopify Dev Dashboard. App phải được Release và Install vào store trước.
              </p>
              <div style={{ padding: 'var(--space-sm)', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--color-warning)' }}>
                <strong style={{color: 'var(--color-warning)'}}>Lưu ý:</strong> Shopify mới không còn bắt buộc hiện <code>shpat_</code> trong UI. AdPilot sẽ gọi token endpoint bằng <code>grant_type=client_credentials</code>.
              </div>
              ...
              <form onSubmit={handleStartAuth} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)', marginTop: 'var(--space-sm)' }}>
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" name="shop" defaultValue="tkww3m-6u.myshopify.com" />
                </div>
